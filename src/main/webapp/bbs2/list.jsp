...
<table width="800" border="0" cellpadding="0" cellspacing="0">
	<tr bgcolor=#A11A20 height="30" style="color:#fefefe;">
		...
		<td width="440" id="bbsLabel" colspan="2" align="center">제목</td>
...
%>
	<tr height=40>
		<td align=center>
			...
			<%if(file1_s.equals("")){ %>
				<td width=20 style="color:gray; font-size:10; text-align:center">no<br>image</td>
			<%}else{ %>
				<td width=20 ><img src = /upload/<%=file1_s %> style="width:40px; height:40px;"></td>
			<%} %>
		<td style="padding-left:10px; padding-bottom:7px; font-size:18px;">
			<!-- 비밀글과 일반글 상세보기 구분 -->
			<%if(gongji.equals("3")){ %>
			<!-- 비밀글이라면 -->
				<%if(session_level != null && (session_level.equals("10") || session_id.equals(id))){ %>
						<!-- (세션상 레벨이 10이거나 세션아이디가 작성자 아이디와 같다면) -->
						<!-- 관리자와 작성자만 볼 수 있도록 처리 -->
					<a href="view.jsp?code=<%=code%>&uid=<%=uid%>&field=<%=field %>&search=<%=search %>&page_now=<%=page_now %>"><%=subject %></a>
						<!-- 관리자와 작성자는 상세보기로 갈 수 있도록 -->
				<%}else{ %>
						<!-- 다른사람이 작성한 비밀글을 누르면? -->
					<span onclick="alert('관리자와 작성자만 볼 수 있...')"><%=subject %></span>
						<!-- 넌 못지나간다! -->
				<%}%>
			<!-- 비밀글과 일반글 상세보기 구분 끗 -->
			<%}else{%>
			<!-- else로 비밀글과 일반글구분, 일반글은 제목을 누르면 상세보기로 가도록-->
				<a href="view.jsp?code=<%=code%>&uid=<%=uid%>&field=<%=field %>&search=<%=search %>&page_now=<%=page_now %>"><%=subject %></a>
			<%}%>
			...
			<%}%>
			<!-- 비밀글일때 글 제목 옆에 나올 아이콘 (뒤에 있기때문에 늦게 나옴) -->
			<%if(signdate2.getTime() >= day2.getTime()){%>
				<img src="img/main_new.gif">
			<%}%>
		<!-- 일반글 상세보기 끗 -->
		</td>
			<%if(rs_normal.getString("name").equals("관리자")){ %>
				<td align=center style="font-weight:bold; color:#4db18b"><%=rs_normal.getString("name") %></td>
			<%}else{%>
				<td align=center><%=rs_normal.getString("name") %></td>
			<%}%>
		<!-- 관리자 볼드체 -->
	...
	<tr><td colsapn=5 bgcolor=#4db18b"></td></tr>
<%
num--;
}
rs_normal.close();
