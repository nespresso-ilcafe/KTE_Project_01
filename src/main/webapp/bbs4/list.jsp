...
%>
	...
	<tr height=22 bgcolor="#FFF6FA">
		...
		<td style="font-weight:bold;color:#A11A20;">
...
String s_normal = "select * from "+code+" where gongji != '1' "+sql_search+"order by fid desc,thread asc limit "+first+","+num_per_page;
//fid 기준으로 묶고 thread기준으로 오름차순으로 만들어서 정렬한다.
...
while(rs_normal.next()){
	...
	String thread = rs_normal.getString("thread");
...
%>
	<tr height=22>
		...
		<td>
			...
			<%}%>
			<!-- 파일첨부시 글 제목 옆에 나올 아이콘 (앞에 있기때문에 먼저 나옴) -->
			<%if(gongji.equals("3")){ %>
		...
		<!-- 일반글 상세보기 끗 -->
			...
			<%if(rs_normal.getString("name").equals("관리자")){ %>
				<td align=center style="font-weight:bold; color:#4db18b"><%=rs_normal.getString("name") %>
			<%}else{%>
